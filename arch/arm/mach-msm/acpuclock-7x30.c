...
#define LG_ACPU_MIN_UV_MV 750U
#define LG_ACPU_MAX_UV_MV 1525U
...
static struct clkctl_acpu_speed acpu_freq_tbl[] = {
	...
	{ 0, 245760, PLL_3,    5, 2,  61440000,  750, VDD_RAW(750) },
	{ 1, 368640, PLL_3,    5, 1,  122800000, 750, VDD_RAW(750) },
	/* AXI has MSMC1 implications. See above. */
	{ 1, 768000, PLL_1,    2, 0,  153600000, 850, VDD_RAW(850) },
	/*
	 * AXI has MSMC1 implications. See above.
	 */
	{ 1, 806400,  PLL_2, 3, 0, UINT_MAX, 875, VDD_RAW(875), &pll2_tbl[0]},
	{ 1, 1024000, PLL_2, 3, 0, UINT_MAX, 1075, VDD_RAW(1075), &pll2_tbl[1]},
	{ 1, 1200000, PLL_2, 3, 0, UINT_MAX, 1075, VDD_RAW(1075), &pll2_tbl[2]},
	{ 1, 1401600, PLL_2, 3, 0, UINT_MAX, 1150, VDD_RAW(1150), &pll2_tbl[3]},
	//{ 1, 1520000, PLL_2, 3, 0, UINT_MAX, 1300, VDD_RAW(1300), &pll2_tbl[4]},
	{ 1, 1612800, PLL_2, 3, 0, UINT_MAX, 1250, VDD_RAW(1250), &pll2_tbl[4]},
	{ 1, 1708800, PLL_2, 3, 0, UINT_MAX, 1300, VDD_RAW(1300), &pll2_tbl[5]},
...
void acpuclk_set_vdd(unsigned int khz, int vdd)
{
	...
	for (i = 0; acpu_freq_tbl[i].acpu_clk_khz; i++)
	{
		...
		{
			if (khz == 0)
				new_vdd = min(max((acpu_freq_tbl[i].vdd_mv + vdd), LG_ACPU_MIN_UV_MV), LG_ACPU_MAX_UV_MV);
			else if (acpu_freq_tbl[i].acpu_clk_khz == khz)
				new_vdd = min(max((unsigned int)vdd, LG_ACPU_MIN_UV_MV), LG_ACPU_MAX_UV_MV);
			else continue;
